...
EspHal::EspHal(int8_t sck, int8_t miso, int8_t mosi)
    : RadioLibHal(GPIO_MODE_INPUT, GPIO_MODE_OUTPUT, 0, 1, GPIO_INTR_POSEDGE,
                  GPIO_INTR_NEGEDGE),
      m_spiSCK(sck), m_spiMISO(miso), m_spiMOSI(mosi) {
  (void)gpio_install_isr_service(ESP_INTR_FLAG_IRAM);
}

void EspHal::init() { spiBegin(); }

void EspHal::term() { spiEnd(); }
...
// Timing
void EspHal::delay(RadioLibTime_t ms) { vTaskDelay(ms / portTICK_PERIOD_MS); }

void EspHal::delayMicroseconds(RadioLibTime_t us) {
  // Note: vTaskDelay() has 1ms resolution, so we need to use a busy-wait loop
  // here Might be a better way to do this? TODO
...
void EspHal::spiBegin() {
  ...
  bus.mosi_io_num = m_spiMOSI;
  bus.miso_io_num = m_spiMISO;
  bus.sclk_io_num = m_spiSCK;
  bus.quadwp_io_num = -1;
  bus.quadhd_io_num = -1;
  ...
  err = spi_bus_add_device(SPI3_HOST, &dev, &m_spi);
  if (err != ESP_OK && err != ESP_ERR_INVALID_STATE) {
...
uint8_t EspHal::spiTransferByte(uint8_t b) {
  ...
  t.tx_data[0] = b;
  esp_err_t err = spi_device_polling_transmit(m_spi, &t);
  if (err != ESP_OK) {
...
void EspHal::spiEnd() {
  if (m_spi) {
    (void)spi_bus_remove_device(m_spi);
    m_spi = nullptr;
  }
